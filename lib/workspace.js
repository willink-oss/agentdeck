...
(function (root, factory) {
  ...
  /* Restore replays the concrete `command` string, so a deck restores correctly
   * without this. What it buys is the UI knowing WHICH profile produced that
   * command, so a restored pane still says "plan mode" instead of going blank.
   * Additive and optional: an older build drops the field and still restores. */
  var PROFILE_KEY = 'profileId';
  ...
  function toConfig(s) {
    ...
    copyOptional(out, s, PROFILE_KEY);
  ...
  function normalize(raw) {
    ...
    for (var i = 0; i < raw.length; i++) {
      ...
      if (c && typeof c === 'object' && typeof c.presetKey === 'string' && c.presetKey) {
        ...
        copyOptional(entry, c, PROFILE_KEY);
        out.push(entry);
      }
    }
    return out;
