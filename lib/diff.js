...
(function (root, factory) {
   ...
   *  from classifyLine (trusted), everything else is escaped user content. */
  function segmentsToHtml(segs) {
    ...
    for (const seg of segs || []) {
      if (seg.parts) {
        ...
        let inner = '';
        for (const p of seg.parts) inner += p.changed ? `<span class="dl-word">${escapeHtml(p.text)}</span>` : escapeHtml(p.text);
        html += `<span class="${seg.cls}">${inner || '&nbsp;'}</span>`;
      } else {
        html += `<span class="${seg.cls}">${escapeHtml(seg.text) || '&nbsp;'}</span>`;
      }
    }
    return html;
  }

  return { classifyLine, diffToSegments, wordDiff, escapeHtml, segmentsToHtml };
}));
