...
(function (root, factory) {
  ...
  var SPINNER = /[⠀-⣿]/;          // braille range (incl. U+2800 blank some TUIs pad with)

  /** Classify the last few terminal lines into prompt/question/working/plain. */
  function classifyTail(lines) {
    var tail = (lines || []).filter(function (l) { return l && l.trim(); }).slice(-4);
    if (!tail.length) return 'plain';
    var last = tail[tail.length - 1].replace(/\s+$/, '');
