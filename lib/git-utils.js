...
(function (root, factory) {
  ...
  /** Only complete SHA-1/SHA-256 object ids are safe persisted commit facts.
   *  Reject short hashes and revision expressions such as HEAD~1. */
  function isFullCommitHash(value) {
    return /^[0-9a-f]{40}(?:[0-9a-f]{24})?$/i.test(String(value == null ? '' : value));
  }

  return { defaultShell, sanitizeBranch, worktreeFolderName, isFullCommitHash };
}));
