...
(function (root, factory) {
  ...
  function markFired(sched, now) {
    ...
    for (var i = 0; i < keys.length; i++) out[keys[i]] = sched[keys[i]];
  ...
  /** Deterministic, dependency-free FNV-1a 32-bit hash -> base36 token. Used to
   *  fold a discriminator into a short, branch-safe ([A-Za-z0-9._-]) suffix. */
   ...
   *  The minute stamp keeps a single schedule's repeated firings from colliding,
   *  but two DISTINCT schedules sharing repo + preset + empty prefix would still
   *  land on the same minute-stamped name. So when a non-empty string
   *  `discriminator` (the schedule's unique id) is supplied, its FNV-1a token is
   *  appended ("…-<token>") to keep same-minute firings of different schedules
   *  apart (#9 / FR-4). With no/empty/non-string discriminator the output is
   *  exactly the legacy 3-arg form, so existing callers are unaffected. */
  function uniqueBranch(prefix, presetKey, now, discriminator) {
    var base = branchSlug(prefix) || branchSlug(presetKey) || 'session';
    var d = new Date(now);
    var stamp = d.getFullYear() + pad2(d.getMonth() + 1) + pad2(d.getDate()) +
      '-' + pad2(d.getHours()) + pad2(d.getMinutes());
    var branch = 'agentdeck/' + base + '-' + stamp;
    if (typeof discriminator === 'string' && discriminator) branch += '-' + hashToken(discriminator);
    return branch;
  }
