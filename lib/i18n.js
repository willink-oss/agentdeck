...
(function (root, factory) {
  ...
  var DICT = {
    ...
    'empty.desc':     { ja: '上のチップからエージェントを起動すると、ここにターミナルが並びます。▾ でプロファイルを選べます。複数立ち上げれば並列で監視できます。', en: 'Launch an agent from the chips above and its terminal appears here. Use ▾ to pick a profile. Run several to watch them in parallel.', zhHans: '从上方的按钮启动智能体，其终端就会显示在这里。点击 ▾ 可选择配置。启动多个即可并行监视。', zhHant: '從上方的按鈕啟動代理，其終端機就會顯示在這裡。點擊 ▾ 可選擇設定檔。啟動多個即可並行監看。', ko: '위쪽 칩에서 에이전트를 실행하면 해당 터미널이 여기에 표시됩니다. ▾ 로 프로필을 고를 수 있습니다. 여러 개를 실행하면 병렬로 모니터링할 수 있습니다.' },
    ...
    'app.confirmClose': { ja: '実行中のセッションが {n} 件あります。ウィンドウを閉じるとすべて終了します。閉じますか？', en: '{n} session(s) are still running. Closing the window ends all of them. Close anyway?', zhHans: '仍有 {n} 个会话在运行。关闭窗口将全部终止。仍要关闭吗？', zhHant: '仍有 {n} 個工作階段在執行。關閉視窗將全部終止。仍要關閉嗎？', ko: '실행 중인 세션이 {n}개 있습니다. 창을 닫으면 모두 종료됩니다. 닫으시겠습니까?' },
    ...
    'presets.editTitle': { ja: '{label} の起動コマンドと起動後コマンド', en: 'Launch and post-launch commands for {label}', zhHans: '{label} 的启动命令与启动后命令', zhHant: '{label} 的啟動指令與啟動後指令', ko: '{label}의 시작 명령 및 실행 후 명령' },
    'presets.cmdHint':   { ja: 'オプションを含めて指定できます（例: claude --model opus）。空欄なら何も自動実行しません。', en: 'Options are allowed (e.g. claude --model opus). Leave empty to auto-run nothing.', zhHans: '可包含选项（例如 claude --model opus）。留空则不自动运行。', zhHant: '可包含選項（例如 claude --model opus）。留空則不自動執行。', ko: '옵션을 포함할 수 있습니다(예: claude --model opus). 비워두면 아무것도 자동 실행하지 않습니다.' },
    ...
    // --- launch profiles (the argument half of a launch command) ---
    'profile.default':  { ja: '標準', en: 'Default', zhHans: '标准', zhHant: '標準', ko: '기본' },
    'profile.continue': { ja: '続きから', en: 'Continue', zhHans: '继续上次', zhHant: '接續上次', ko: '이어서' },
    'profile.plan':     { ja: 'plan モード', en: 'Plan mode', zhHans: 'plan 模式', zhHant: 'plan 模式', ko: 'plan 모드' },
    'profile.auto':     { ja: '自動編集', en: 'Auto-edit', zhHans: '自动编辑', zhHant: '自動編輯', ko: '자동 편집' },
    'profile.yolo':     { ja: '自動承認', en: 'Auto-approve', zhHans: '自动批准', zhHant: '自動核准', ko: '자동 승인' },
    'profile.label':    { ja: 'プロファイル', en: 'Profile', zhHans: '配置', zhHant: '設定檔', ko: '프로필' },
    'profile.danger':   { ja: '危険', en: 'Risky', zhHans: '危险', zhHant: '危險', ko: '위험' },
    'profile.dangerHint': { ja: '確認なしでファイル編集とコマンド実行を許可します', en: 'Lets the agent edit files and run commands without asking', zhHans: '允许智能体在不询问的情况下编辑文件并执行命令', zhHant: '允許代理在不詢問的情況下編輯檔案並執行指令', ko: '확인 없이 파일 편집과 명령 실행을 허용합니다' },
    'profile.dangerConfirm': { ja: '「{label}」は確認なしでファイル編集とコマンド実行を行います。\n\n{command}\n\n起動しますか？', en: '“{label}” will edit files and run commands without asking.\n\n{command}\n\nLaunch anyway?', zhHans: '“{label}”将在不询问的情况下编辑文件并执行命令。\n\n{command}\n\n仍要启动吗？', zhHant: '「{label}」將在不詢問的情況下編輯檔案並執行指令。\n\n{command}\n\n仍要啟動嗎？', ko: '“{label}”은(는) 확인 없이 파일을 편집하고 명령을 실행합니다.\n\n{command}\n\n실행하시겠습니까?' },
    'profile.schedBlocked': { ja: '無人で発火するスケジュールには「{label}」を選べません。', en: '“{label}” cannot be used for an unattended scheduled launch.', zhHans: '无人值守的定时启动无法使用“{label}”。', zhHant: '無人值守的排程啟動無法使用「{label}」。', ko: '무인으로 실행되는 예약에는 “{label}”을(를) 사용할 수 없습니다.' },
    'profile.edited':   { ja: '変更済み', en: 'Edited', zhHans: '已修改', zhHant: '已修改', ko: '수정됨' },
    'launch.newSession': { ja: '＋ 新しいセッション', en: '+ New session', zhHans: '＋ 新建会话', zhHant: '＋ 新增工作階段', ko: '＋ 새 세션' },
    'launch.title':      { ja: 'セッションを起動', en: 'Launch a session', zhHans: '启动会话', zhHant: '啟動工作階段', ko: '세션 실행' },
    'launch.search':     { ja: 'セッションを検索', en: 'Search sessions', zhHans: '搜索会话', zhHant: '搜尋工作階段', ko: '세션 검색' },
    'chip.launchIn':     { ja: '{agent} を {repo} で起動', en: 'Launch {agent} in {repo}', zhHans: '在 {repo} 中启动 {agent}', zhHant: '在 {repo} 中啟動 {agent}', ko: '{repo}에서 {agent} 실행' },
    'chip.launchHome':   { ja: '{agent} を起動', en: 'Launch {agent}', zhHans: '启动 {agent}', zhHant: '啟動 {agent}', ko: '{agent} 실행' },
    'chip.profileMenu':  { ja: '{agent} のプロファイルを選ぶ', en: 'Choose a profile for {agent}', zhHans: '选择 {agent} 的配置', zhHant: '選擇 {agent} 的設定檔', ko: '{agent}의 프로필 선택' },
    'repo.other':        { ja: 'その他', en: 'Other', zhHans: '其他', zhHant: '其他', ko: '기타' },
    'pane.repoTitle':    { ja: 'リポジトリ: {repo}', en: 'Repository: {repo}', zhHans: '仓库：{repo}', zhHant: '儲存庫：{repo}', ko: '저장소: {repo}' },
    'filter.all':        { ja: 'すべて', en: 'All', zhHans: '全部', zhHant: '全部', ko: '전체' },
    'filter.attention':  { ja: '要対応', en: 'Needs you', zhHans: '需处理', zhHant: '需處理', ko: '확인 필요' },
    'filter.exited':     { ja: '終了', en: 'Exited', zhHans: '已结束', zhHant: '已結束', ko: '종료됨' },
    'sidebar.resize':    { ja: 'サイドバーの幅を変更（ドラッグ / ←→ キー / ダブルクリックで既定）', en: 'Resize the sidebar (drag, arrow keys, double-click to reset)', zhHans: '调整侧边栏宽度（拖动 / 方向键 / 双击恢复默认）', zhHant: '調整側邊欄寬度（拖曳 / 方向鍵 / 雙擊還原預設）', ko: '사이드바 너비 조절 (드래그 / 화살표 키 / 더블클릭으로 기본값)' },
    'filter.title':      { ja: '状態で絞り込む', en: 'Filter by state', zhHans: '按状态筛选', zhHant: '依狀態篩選', ko: '상태로 필터' },
    'diff.title':        { ja: '変更差分', en: 'Changes', zhHans: '变更差异', zhHant: '變更差異', ko: '변경 사항' },
    'palette.title':     { ja: 'セッションへ移動', en: 'Jump to a session', zhHans: '跳转到会话', zhHant: '跳至工作階段', ko: '세션으로 이동' },
    'profile.reset':    { ja: '既定に戻す', en: 'Reset to default', zhHans: '恢复默认', zhHant: '還原預設', ko: '기본값으로' },
    // --- scheduled launches ---
