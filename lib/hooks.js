...
/* What Shift+Enter should send to the PTY.
 *
 * Enter submits. Agents that accept multi-line input need a second key for
 * "newline, don't submit", and each terminal emulator has to be taught it —
 * which is what Claude Code's /terminal-setup does. Backslash-then-newline is
 * the continuation the agent's input box (and every shell) already understands,
 * so it works whether or not the agent knows anything about Shift+Enter.
 *
 * CR only, not CRLF. /terminal-setup writes "\\\r\n" into VS Code's
 * keybindings, but that goes through sendSequence, not straight to a PTY — and a
 * PTY reads CR and LF as two line terminators. Sending both continues the line
 * and then immediately submits the empty continuation, which is exactly what a
 * real zsh did when this was tried: `echo A\\` , `> `, then `A`.
 *
 * Kept here as a named constant so the day an agent wants something else, there
 * is one place to look. */
const SHIFT_ENTER = '\\\r';

/** True when this keydown is the plain Shift+Enter chord (no other modifier). */
function isShiftEnter(e) {
  return !!e && e.type === 'keydown' && e.key === 'Enter'
    && !!e.shiftKey && !e.ctrlKey && !e.metaKey && !e.altKey;
}

/** Turn dropped file paths into something safe to type at an agent's prompt.
 *
 * Agents accept a path as plain text, so this is a paste, not a shell command —
 * but the text still lands in a PTY, so a path containing a newline could submit
 * a line the user never typed. Paths with whitespace or shell metacharacters are
 * single-quoted (with embedded quotes escaped the POSIX way), and control
 * characters are refused outright rather than sanitised into something that
 * looks like a different file. */
function quotePath(p) {
  // paths arrive from webUtils.getPathForFile(), which returns a string; anything
  // else is a programming error, and coercing it would type something plausible
  if (typeof p !== 'string') return '';
  const raw = p;
  if (!raw) return '';
  if (/[\x00-\x1F\x7F]/.test(raw)) return ''; // a newline here would submit the prompt
  if (!/[\s'"\\|&;<>()$`*?\[\]#~]/.test(raw)) return raw;
  return "'" + raw.replace(/'/g, "'\\''") + "'";
}

/** The text to type for a set of dropped paths: quoted, deduped, space-joined. */
...
return {
  ...
  SHIFT_ENTER, isShiftEnter, quotePath, dropText,
  };
}));
